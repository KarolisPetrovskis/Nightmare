...
export default function WorkerManagement() {
  ...
  const fetchEmployees = async () => {
    ...
    try {
      ...
      // REMOVED: Don't filter out SuperAdmin's own record
      // if (currentUserType === 4) {
      //   filteredData = filteredData.filter((worker: ApiWorker) => worker.nid !== userId);
      // }
  ...
  const handleNew = () => {
    ...
    setFormData({
      ...
      userType: 1, // Default to staff
      businessId:
        currentUserType === 4 && currentUserBusinessId
  ...
  const handleMoreDetails = (w: ApiWorker) => {
    ...
    const isEditingOwnProfile = w.nid === userId;
    ...
    setFormData({
      ...
      businessId: w.businessId != null ? w.businessId : undefined,
      bossId: w.bossId ? w.bossId.toString() : userId ? userId.toString() : '',
      // If SuperAdmin is editing their own profile, force userType to 4
      userType: isEditingOwnProfile ? 4 : w.userType || 1,
    });
    console.log('User type: ', currentUserType);
    console.log('bus id: ', currentUserBusinessId);
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    // For SuperAdmin, check if businessId is set manually or use their own businessId    let targetBusinessId;
    if (currentUserType === 4) {
      // SuperAdmin: use the form's businessId if provided, otherwise use their own businessId
      targetBusinessId = formData.businessId || currentUserBusinessId;
    } else {
      // Non-SuperAdmin: use the businessId from context
    ...
    try {
      ...
      const targetBusinessId =
        currentUserType === 4 && formData.businessId
          ? formData.businessId
          : businessId;
      ...
      if (currentWorker) {
        ...
        const updatePayload: any = {
          ...
          // If editing own profile, force userType to 4 for SuperAdmin
          userType:
            currentWorker.nid === userId && currentUserType === 4
              ? 4
              : formData.userType,
      ...
      showSnackbar(
        error instanceof Error ? error.message : 'Error saving employee',
        'error'
      );
      console.error('Error:', error);
    } finally {
  ...
  const handleDelete = async (worker: ApiWorker) => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${worker.name} ${worker.surname}?`
      )
    )
      return;
        ...
        <div className="modal-overlay" onClick={handleCloseModal}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <h3 className="modal-title">
              {currentWorker ? 'Edit Employee' : 'Create New Employee'}
            </h3>
            ...
            <div className="info-box">
              <label>
                Password {currentWorker ? '(optional to keep current)' : '*'}
              </label>
              <input
                type="password"
            ...
            <div className="info-box">
              ...
              {/* Hide dropdown if SuperAdmin is editing their own profile */}
              {currentWorker?.nid === userId && currentUserType === 4 ? (
                ...
                >
                  ...
                  {currentUserType === 4 && (
                    <option value={3}>Owner (Business Owner)</option>
                  )}
                </select>
              )}
            </div>
            ...
            {currentUserType === 4 && (
              <div className="info-box">
                ...
                />
                {currentUserBusinessId && !formData.businessId && (
                  <p>
                    Leave blank to use your current business ID:{' '}
                    {currentUserBusinessId}
                  </p>
                )}
              </div>
